...
data class ExerciseMetrics(
    ...
    fun update(latestMetrics: DataPointContainer): ExerciseMetrics {
        Log.d("HEART_RATE_BPM", latestMetrics.getData(DataType.HEART_RATE_BPM).lastOrNull()?.value.toString())
        Log.d("DISTANCE_TOTAL", latestMetrics.getData(DataType.DISTANCE_TOTAL)?.total.toString())
        Log.d("ABSOLUTE_ELEVATION", latestMetrics.getData(DataType.ABSOLUTE_ELEVATION).lastOrNull()?.value.toString())
        Log.d("ELEVATION_GAIN_TOTAL", latestMetrics.getData(DataType.ELEVATION_GAIN_TOTAL)?.total.toString())
        Log.d("LOCATION", latestMetrics.getData(DataType.LOCATION).lastOrNull()?.value.toString())
        ...
        return copy(
            ...
            stepsTotal = latestMetrics.getData(DataType.STEPS).lastOrNull()?.value ?: stepsTotal,
